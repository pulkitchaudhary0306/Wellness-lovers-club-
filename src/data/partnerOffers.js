 ...
 * Each partner possesses a unique slug and partner ID.
 * Each offer strictly belongs to its designated partner.
 */
...
export function getPartnerBySlug(slug) {
  ...
  return PARTNERS_DATA.find((p) => p.slug === normalizedSlug) || null;
}
...
export function getPartnerById(id) {
  ...
  return PARTNERS_DATA.find((p) => p.id === numId) || null;
}
